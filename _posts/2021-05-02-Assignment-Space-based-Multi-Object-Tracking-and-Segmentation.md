...
I find the use of structured learning to figure out the weights ($\lambda_{\text{iou}}$, $\lambda_{\text{app}}$ and $\lambda_{\text{dis}}$) used in determining the cost of assigning a detection at $t-1$ to a detection at $t$ interesting. More specifically: 
