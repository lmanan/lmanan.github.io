...
An alternative to this is provided by Deep Snake, where $N$ points are sampled on the initial contour and the GT contour. The correspondences are established by assigning the point on the GT contour closest to the top extremal point on the initial contour. Next, the features at these points are concatenated with the actual coordinates and then passed through a graph convolutional network. The difference from a typical GCN is that 
circular convolution is performed (the difference from a vanilla GCN is not very clear to me ...).
